...
RC IndexScanPhysicalOperator::next()
{
  ...
  if (index_scanner_->next_entry(&rid, false) == RC::RECORD_EOF)
  {
  ...
  {
    Record *rec = nullptr;
    const RID* rid_ptr = &rid;
    rc = record_handler_->get_record(record_page_handler_, rid_ptr, true, &current_record_);
    if(rc != RC::SUCCESS)
    {
      return rc;
    }
